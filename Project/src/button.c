...
#ifdef HOME_VERSION
    // sleeping mode
    Msg_DevOnOffDelay(DEVICE_SW_OFF,SLEEP_BR,MINUTE_UNIT,SLEEP_TIME,TRUE);
#else
#ifdef ENABLE_SDTM
    // Pure Green
...
void btn_double_long_hold_press(uint8_t _btn1, uint8_t _btn2)
{
  ...
  if( _btn1 == keylstCenter && _btn2 == keylstFn4 ) {
    Msg_Pairing();
  }
...
void check_track_double_button(void)
{
  if( btn_is_pushed[keylstFLASH] == TRUE && btn_is_pushed[keylstFn1] == TRUE )
  {
  ...
  } else if( btn_is_pushed[keylstFLASH] == TRUE && btn_is_pushed[keylstFn2] == TRUE ) {
    timer_stop(m_timer_id_btn_detet[keylstFn2]);  // Disable btn2_timer when tracking double hold.
    m_btn_timer_status[keylstFLASH] = BUTTON_STATUS_DOUBLE_TRACK;
    m_btn_timer_status[keylstFn2] = BUTTON_STATUS_INIT;
    double_button_track = TRUE;
    timer_start(m_timer_id_btn_detet[keylstFn2], BUTTON_DOUBLE_BTN_TRACK_DURATION);  //3 s
  } else if( btn_is_pushed[keylstCenter] == TRUE && btn_is_pushed[keylstFn4] == TRUE ) {
    timer_stop(m_timer_id_btn_detet[keylstFn4]);  // Disable btn2_timer when tracking double hold.
    ...
    if (double_button_track == TRUE)
    {
      ...
      timer_stop(m_timer_id_btn_detet[keylstFn2]);
