  ...
  case C_PRESENTATION:
    if( _sensor == S_DIMMER ) {
      if( _isAck ) {
        ...
        if(IS_REMOTE_NODEID(_type) || IS_GROUP_NODEID(_type) ) {
...
void Msg_Pairing() {
  build(CurrentDeviceID,NCF_DEV_SET_REMOTE, C_INTERNAL, I_CONFIG, 0, 1);
  moSetLength(1);
  moSetPayloadType(P_BYTE);
  sndMsg.payload.data[0] = 0;     
...
void Msg_DevOnOffDelay(uint8_t _sw,uint8_t _br,uint8_t _unit,uint8_t time,bool isGradual)
{
  ...
  if(isGradual)
  {
    sndMsg.payload.data[1] = (1<<4) | _unit;
  }
  else
  {
